...
export function Sidebar({ isMobileOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <aside 
      ...
      className={`fixed left-0 top-0 h-screen bg-[#25175d] text-white flex flex-col transition-all duration-300 ease-in-out z-50 shadow-2xl 
        ${isExpanded ? "w-64" : "w-20"}
        ${isMobileOpen ? "translate-x-0 w-64" : "-translate-x-full md:translate-x-0"}
      `}
    >
      <div className={`p-5 border-b border-white/5 overflow-hidden transition-all duration-300 flex justify-between md:justify-center items-center ${isExpanded ? "px-6" : "px-4"}`}>
        <div className="flex-shrink-0 w-16 h-16 flex items-center justify-center bg-white rounded-2xl shadow-lg border border-slate-100 p-1 hover:scale-110 hover:-rotate-3 hover:shadow-xl transition-all duration-300 cursor-pointer">
          <img src="/moon-logo.png" alt="Moon Travel Co." className="w-full h-full object-contain drop-shadow-sm" />
        </div>
        {/* Mobile Close Button */}
        <button 
          onClick={onClose}
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `relative flex items-center rounded-xl transition-all duration-300 group ${
                      ...
                      isActive
                        ? "bg-[#512DDB] text-white shadow-sm"
                ...
                >
                  {({ isActive }) => (
                    <>
                      ...
                      <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 rounded-r-full bg-[#D2C3F7] shadow-[0_0_12px_#D2C3F7] transition-all duration-500 ease-in-out ${
                        isActive ? 'h-8 opacity-100 translate-x-0' : 'h-0 opacity-0 -translate-x-full'
                      }`} />
                      
                      <div className={`flex-shrink-0 transition-transform duration-300 group-hover:scale-110 ${isActive ? 'text-[#D2C3F7]' : ''}`}>
                        <link.icon size={22} />
                      </div>
      ...
      <div className={`p-4 border-t border-white/5 transition-all duration-300 ${isExpanded ? "" : "items-center"}`}>
        <div className={`flex items-center gap-3 mb-4 transition-all duration-300 ${isExpanded ? "" : "justify-center"}`}>
          <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-[#512DDB] flex items-center justify-center font-bold text-white shadow-lg">
