    ...
    >
      <div className="space-y-4">
        <div className="flex flex-col items-center text-center p-4 bg-gradient-to-b from-accent/5 to-transparent rounded-2xl border border-accent/5 mb-2">
          <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center text-primary text-2xl font-semibold overflow-hidden border-4 border-white shadow-sm">
            {responsable.name.charAt(0)}
          </div>
          <h2 className="text-lg font-bold text-primary mt-2">{responsable.name}</h2>
        ...
        <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
          <div><span className="text-gray-500 text-sm block">Tipo Doc:</span> <span className="font-medium">{responsable.docType}</span></div>
          <div><span className="text-gray-500 text-sm block">Numero:</span> <span className="font-medium">{responsable.docNumber}</span></div>
          <div><span className="text-gray-500 text-sm block">Telefono:</span> <span className="font-medium">{responsable.phone}</span></div>
          <div className="min-w-0"><span className="text-gray-500 text-sm block">Correo:</span> <span className="font-medium block break-all">{responsable.email}</span></div>
          <div><span className="text-gray-500 text-sm block">F. Nacimiento:</span> <span className="font-medium">{responsable.birthDate ? formatDate(responsable.birthDate) : 'N/A'}</span></div>
          <div><span className="text-gray-500 text-sm block">Registro:</span> <span className="font-medium">{formatDate(responsable.creadoAt)}</span></div>
        </div>
        <div>
              ...
              <tbody className="divide-y divide-gray-100">
                {responsableSales.map(s => (
                  <tr key={s.id} className="hover:bg-gray-50/50">
                    <td className="p-2 font-mono text-gray-500">#{s.id.toString().padStart(4, '0')}</td>
